...
export default function ProductList() {
    ...
    const getVinyls = async () => {
        try {
            const fetchVinylList = await vinylApi.getVinylList(currentPage,sortByAndDir);
    ...
    useEffect(() => {
        getVinyls();
    },[currentPage,sortByAndDir]);

    const handleFirstPage = () => {
        let firstPage = 1;
        if (currentPage > firstPage) {
            if (sortByAndDir !== null) {
                history.push(`/products?page=${firstPage}&${sortByAndDir}`);
            } else history.push(`/products?page=${firstPage}`);
        }
    }
    ...
    const handlePrevPage = () => {
        ...
        if (currentPage > prevPage) {
            if (sortByAndDir !== null) {
                history.push(`/products?page=${currentPage - prevPage}&${sortByAndDir}`);
            } else history.push(`/products?page=${currentPage - prevPage}`);
        }
    }
    ...
    const handleLastPage = () => {
        ...
        if (currentPage < condition) {
            if (sortByAndDir !== null) {
                history.push(`/products?page=${condition}&${sortByAndDir}`);
            } else history.push(`/products?page=${condition}`);
        }
    }
    ...
    const handleNextPage = () => {
        if (currentPage < Math.ceil(totalElements / vinylsPerPage)) {
            if (sortByAndDir !== null) {
                history.push(`/products?page=${currentPage + 1}&${sortByAndDir}`);
            } else history.push(`/products?page=${currentPage + 1}`);
        }
    }
    ...
    const handleChangePage = (event :  React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (sortByAndDir !== null) {
            history.push(`/products?page=${targetPage}&${sortByAndDir}`);
        } else history.push(`/products?page=${targetPage}`);
    }
        ...
        <div className="home_produce">
            <div className="home_produce__category">
                <div className="home_produce__category__sortBy">
                    Sắp xếp theo:
                    <form id="form_sort">
                        <select name="sort__selector" defaultValue={sortByAndDir===null?"sort=id&direction=asc":sortByAndDir} onChange={handleSortProducts} className="sort__selector">
                            ...
                            {/*<option th:if="${s==null}" th:value="${#httpServletRequest.requestURI}+'?sortOption=realPricedesc'" th:selected="${sortOption=='realPricedesc'}">Giá cao đến thấp</option>*/}
                            {/*<option th:if="${s!=null}" th:value="'/all-vinyl-product?s='+${s}+'&sortOption=realPricedesc'" th:selected="${sortOption=='realPricedesc'}">Giá cao đến thấp</option>*/}
                        </select>
                    </form>
                </div>
                <div>
